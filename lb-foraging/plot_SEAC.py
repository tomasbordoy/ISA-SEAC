...
if full_plot:
    ...
    plt.figure()
    test = Plot()
    test.plot(rewareds_list=reward, episode_list=episode, name="NN-NN", color=colors[0], title="Learning curve",
              ylabel="Reward", k=50)
    test.save_plot(name="learningcurve.png")
    plt.figure()
    test = Plot()
    test.plot(rewards_list=actor, episode_list=episode, name="NN-NN", color=colors[1], title="Actor loss curve",
              ylabel="Loss", k=50)
    test.save_plot(name="actorloss.png")
    plt.figure()
    test = Plot()
    test.plot(rewards_list=critic, episode_list=episode, name="NN-NN", color=colors[2], title="Critic loss curve",
              ylabel="Loss", k=50)
