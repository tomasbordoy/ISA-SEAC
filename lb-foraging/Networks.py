...
class NN_Critic(nn.Module):
    def __init__(self,input_dim, nn_type:str="NN",mu:float=0,sigma:float=0.1):
        ...
        if nn_type == "NN":
            self.layer1 = _init(nn.Linear(in_features=input_shape, out_features=self.hidden_size))  #100
            ...
            # self.layer1 = (nn.Linear(in_features=input_shape, out_features=self.hidden_size))  # 100
            # self.layer2 = (nn.Linear(in_features=self.hidden_size, out_features=self.hidden_size))
            # self.layer3 = (nn.Linear(in_features=self.hidden_size, out_features=1))

        elif nn_type=="BNN":
            self.layer1 = _init_bayesian(bnn.BayesLinear(prior_mu=self.mu, prior_sigma=self.sigma, in_features=input_shape, out_features=self.hidden_size))
            self.layer2 = _init_bayesian(bnn.BayesLinear(prior_mu=self.mu,prior_sigma=self.sigma,in_features=self.hidden_size, out_features=self.hidden_size))
            self.layer3 = _init_bayesian(bnn.BayesLinear(prior_mu=self.mu,prior_sigma=self.sigma,in_features=self.hidden_size, out_features=1))
        else:
            raise Exception('Incorrect nn_type')
