...
class NN_Actor(nn.Module):
    def __init__(self, input_dim, output_dim, nn_type: str, mu: float = 0, sigma: float = 0.1):
        ...
        output_shape = np.array(output_dim)[0].n
        ...
        if nn_type == "NN":
            self.layer1 = _init1(nn.Linear(in_features=input_shape, out_features=self.hidden_size))
